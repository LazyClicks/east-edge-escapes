...
const Root = component$<PropsOf<"div">>((props) => {
  return (
    <div
      {...props}
      class={cn(
        "rounded-base border-[2px] border-[#ECECEC] bg-card text-card-foreground shadow-inner",
        props.class,
      )}
    >
      <Slot />
...
const Special = component$<PropsOf<"div">>(({ ...props }) => {
  return (
    <div {...props} class={cn("flex items-center px-4 py-6 pt-0", props.class)}>
      <Slot />
    </div>
  );
});
